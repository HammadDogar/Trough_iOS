//
//  MyTruckEventViewController.swift
//  Trough
//
//  Created by Imed on 07/10/2021.
//

import UIKit

class MyTruckEventViewController: BaseViewController {
...
extension MyTruckEventViewController : UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: "EventTableViewCell", for: indexPath) as? EventTableViewCell{
            ...
            cell.mainViewhieght.constant = 310
            cell.addTruckToEventButton.isHidden = false
...
extension MyTruckEventViewController {
...
extension MyTruckEventViewController :  EventTableViewCellDelegate {
...
extension  MyTruckEventViewController : EditedEventViewDelegate{
...
extension MyTruckEventViewController {
    func selectTruck(){
        let params = [
            "eventId"       : self.currentEvent,
            "truckIds"      : self.truckString,
            "isInviting"    : true
        ] as [String : Any]
                ...
                case .Success :
                    GCD.async(.Main) {
                        if let data = serviceResponse.data as? Bool{
                            if data == true{
                                let alert = UIAlertController(title: "Success", message: "Selected truck added to this event successfully", preferredStyle: UIAlertController.Style.alert)
